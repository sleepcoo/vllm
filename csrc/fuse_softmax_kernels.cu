 ...
 * Copyright (c) 2023, sleepcoo@gmail.com
...
template <> __inline__ __device__ double Exp<double>(double x) {
  return exp(x);
}
...
template <> __inline__ __device__ double Log<double>(double x) {
  return log(x);
}
...
template <> struct DefaultComputeType<double> { using type = float; };
template <> struct DefaultComputeType<c10::Half> { using type = float; };
...
#pragma unroll
      for (int pack_id = 0; pack_id < num_packs; ++pack_id) {
        ...
        if (!padding || col < cols) {
          load.template load<pack_size>(row_buf + pack_offset, row + row_id,
                                        col);
#pragma unroll
          for (int i = 0; i < pack_size; ++i) {
            thread_max[row_id] =
                max(thread_max[row_id], row_buf[pack_offset + i]);
          }
        } else {
#pragma unroll
          for (int i = 0; i < pack_size; ++i) {
...
#pragma unroll
    for (int row_id = 0; row_id < rows_per_access; ++row_id) {
      warp_max[row_id] = WarpAllReduce<MaxOp, ComputeType, thread_group_width>(
          thread_max[row_id]);
    }
...
#pragma unroll
    for (int row_id = 0; row_id < rows_per_access; ++row_id) {
      warp_sum[row_id] = WarpAllReduce<SumOp, ComputeType, thread_group_width>(
          thread_sum[row_id]);
    }
#pragma unroll
    for (int row_id = 0; row_id < rows_per_access; ++row_id) {
...
#pragma unroll
      for (int i = 0; i < num_packs; ++i) {
        ...
        if (!padding || col < cols) {
          store.template store<pack_size>(row_buf + i * pack_size, row + row_id,
                                          col);
        }
      }
    }
...
template <typename LOAD, typename STORE, typename ComputeType, int pack_size,
          int cols_per_thread, int thread_group_width, int rows_per_access,
          bool padding, Algorithm algorithm>
inline cudaError_t LaunchSoftmaxWarpImpl(cudaStream_t stream, LOAD load,
...
template <typename LOAD, typename STORE, typename ComputeType, int pack_size,
          ...
          Algorithm algorithm>
inline cudaError_t
DispatchSoftmaxWarpImplPadding(cudaStream_t stream, LOAD load, STORE store,
                               const int64_t rows, const int64_t cols) {
  if (cols == cols_per_thread * thread_group_width) {
    return LaunchSoftmaxWarpImpl<LOAD, STORE, ComputeType, pack_size,
                                 cols_per_thread, thread_group_width,
...
struct DispatchSoftmaxWarpImplPackSize {
  cudaError_t operator()(cudaStream_t stream, LOAD load, STORE store,
                         const int64_t rows, const int64_t cols) {
    if (cols % 2 == 0) {
      return DispatchSoftmaxWarpImplCols<LOAD, STORE, ComputeType, 2,
                                         algorithm>(stream, load, store, rows,
                                                    cols);
    } else {
      return DispatchSoftmaxWarpImplCols<LOAD, STORE, ComputeType, 1,
                                         algorithm>(stream, load, store, rows,
                                                    cols);
    }
  }
...
template <typename LOAD, typename STORE, typename ComputeType,
          Algorithm algorithm>
inline cudaError_t DispatchSoftmaxWarpImpl(cudaStream_t stream, LOAD load,
                                           STORE store, const int64_t rows,
                                           const int64_t cols) {
...
#pragma unroll
    ...
    const ComputeType row_max =
        BlockAllReduce<MaxOp, ComputeType, block_size>(thread_max);
...
template <typename LOAD, typename STORE, typename ComputeType, int pack_size,
          int block_size, Algorithm algorithm>
inline cudaError_t
LaunchSoftmaxBlockSMemImpl(cudaStream_t stream, LOAD load, STORE store,
  ...
  SoftmaxBlockSMemImpl<LOAD, STORE, ComputeType, pack_size, block_size,
                       algorithm>
...
template <typename LOAD, typename STORE, typename ComputeType, int pack_size,
          Algorithm algorithm>
inline cudaError_t
TryDispatchSoftmaxBlockSMemImplBlockSize(cudaStream_t stream, LOAD load,
                                         STORE store, const int64_t rows,
  ...
  int max_active_blocks_conf_1;
  {
    cudaError_t err = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        ...
        SoftmaxBlockSMemImpl<LOAD, STORE, ComputeType, pack_size,
...
struct TryDispatchSoftmaxBlockSMemImplPackSize {
  cudaError_t operator()(cudaStream_t stream, LOAD load, STORE store,
                         ...
                         bool *success) {
    if (cols % 2 == 0) {
      return TryDispatchSoftmaxBlockSMemImplBlockSize<LOAD, STORE, ComputeType,
                                                      2, algorithm>(
...
template <typename LOAD, typename STORE, typename ComputeType,
          Algorithm algorithm>
inline cudaError_t
TryDispatchSoftmaxBlockSMemImpl(cudaStream_t stream, LOAD load, STORE store,
                                const int64_t rows, const int64_t cols,
                                bool *success) {
  return TryDispatchSoftmaxBlockSMemImplPackSize<LOAD, STORE, ComputeType,
                                                 algorithm>()(
      stream, load, store, rows, cols, success);
}
...
template <typename LOAD, typename STORE, typename ComputeType, int pack_size,
          int block_size, Algorithm algorithm>
__global__ void SoftmaxBlockUncachedImpl(LOAD load, STORE store,
                                         const int64_t rows,
                                         const int64_t cols) {
  const int tid = threadIdx.x;
  assert(cols % pack_size == 0);
...
template <typename LOAD, typename STORE, typename ComputeType, int pack_size,
          Algorithm algorithm>
inline cudaError_t
LaunchSoftmaxBlockUncachedImpl(cudaStream_t stream, LOAD load, STORE store,
                               const int64_t rows, const int64_t cols) {
  ...
  SoftmaxBlockUncachedImpl<LOAD, STORE, ComputeType, pack_size, block_size,
                           algorithm>
...
struct DispatchSoftmaxBlockUncachedImplPackSize {
  cudaError_t operator()(cudaStream_t stream, LOAD load, STORE store,
                         const int64_t rows, const int64_t cols) {
    if (cols % 2 == 0) {
      return LaunchSoftmaxBlockUncachedImpl<LOAD, STORE, ComputeType, 2,
                                            algorithm>(stream, load, store,
                                                       rows, cols);
    } else {
      return LaunchSoftmaxBlockUncachedImpl<LOAD, STORE, ComputeType, 1,
                                            algorithm>(stream, load, store,
                                                       rows, cols);
    }
  }
...
template <typename LOAD, typename STORE, typename ComputeType,
          Algorithm algorithm>
inline cudaError_t
DispatchSoftmaxBlockUncachedImpl(cudaStream_t stream, LOAD load, STORE store,
                                 const int64_t rows, const int64_t cols) {
  return DispatchSoftmaxBlockUncachedImplPackSize<LOAD, STORE, ComputeType,
                                                  algorithm>()(
      stream, load, store, rows, cols);
}
...
DispatchSoftmax(cudaStream_t stream, LOAD load, STORE store, const int64_t rows,
                const int64_t cols) {
  if (cols < 1024) {
    return DispatchSoftmaxWarpImpl<LOAD, STORE, ComputeType,
                                   Algorithm::kSoftmax>(stream, load, store,
                                                        rows, cols);
  } else {
    bool dispatch_smem_impl_success;
    {
      cudaError_t err =
          TryDispatchSoftmaxBlockSMemImpl<LOAD, STORE, ComputeType,
                                          Algorithm::kSoftmax>(
...
inline typename std::enable_if<!std::is_same<ComputeType, double>::value,
                               cudaError_t>::type
DispatchLogSoftmax(cudaStream_t stream, LOAD load, STORE store,
                   const int64_t rows, const int64_t cols) {
  if (cols <= 1024) {
    return DispatchSoftmaxWarpImpl<LOAD, STORE, ComputeType,
...
template <typename SRC, typename ComputeType> struct TemperatureLoad {
  TemperatureLoad(const SRC *src, const ComputeType *temperature,
                  int64_t row_size)
      : src(src), temperature(temperature), row_size(row_size) {}
  template <int N>
  __device__ void load(ComputeType *dst, int64_t row, int64_t col) const {
    Pack<SRC, N> pack;
    const int64_t offset = (row * row_size + col) / N;
    pack.storage = *(reinterpret_cast<const PackType<SRC, N> *>(src) + offset);
    ComputeType temp = temperature[row];
#pragma unroll
    for (int i = 0; i < N; ++i) {
      dst[i] = __fdividef(__half2float(pack.elem[i]), temp);
    }
  }
  const SRC *src;
  const ComputeType *temperature;
  int64_t row_size;
};

template <typename SRC, typename DST, typename ComputeType>
void LaunchFuseSoftmaxKernel(cudaStream_t stream, DST *out, const SRC *input,
                             const ComputeType *temperature, const int64_t rows,
                             const int64_t cols) {
  TemperatureLoad<SRC, ComputeType> load(input, temperature, cols);
  DirectStore<ComputeType, DST> store(out, cols);
...
void fuse_softmax(torch::Tensor &out,           // [batch_size, hidden_size]
                  torch::Tensor &input,         // [batch_size, hidden_size]
                  torch::Tensor &temperature) { // [batch_size]

  int batch_size = input.size(0);
  int hidden_size = input.size(1);
  using ComputeType = typename vllm::DefaultComputeType<float>::type;
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();

  vllm::LaunchFuseSoftmaxKernel<__half, float, ComputeType>(
      stream, out.data_ptr<float>(),
      reinterpret_cast<__half *>(input.data_ptr<at::Half>()),
      temperature.data_ptr<float>(), batch_size, hidden_size);
}
